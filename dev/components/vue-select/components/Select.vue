...
<template>
	<div class="dropdown v-select" :class="dropdownClasses">
		<div ref="toggle" class="dropdown-toggle" v-bind:class="{multi: multiple}" @mousedown.prevent="toggleDropdown">
        <span class="form-control" v-if="!searchable && isValueEmpty">
          {{ placeholder }}
        </span>

		<div style="display: flex; flex-flow: row no-wrap; width:100%; min-height:2.25rem; height:100%; padding: 2px;">
		<div ref="selectedList" style="flex-grow: 1; height:100%; min-height:2.1rem; align-self: center">
        ...
        <div class="open-indicator" style="min-width: 30px; max-width:30px; align-self: center; text-align:right;">
        	<template v-if="multiple">
			<a ref="openIndicator" class="tiny primary button">
			<i v-show="!loading" class="fa fa-plus" aria-hidden="true"></i>
			<slot name="spinner">
				<i class="fa fa-circle-o-notch fa-spin fa-lg spinner" v-show="loading"></i>
			</slot>
			</a>
			</template>
			<template v-else>
		...
		<input
							...
							:style="{ width: '100%' }"
			>
		<ul ref="dropdownMenu" v-show="open" :transition="transition" class="dropdown-menu-list" :style="{ 'max-height': maxHeight }">
			...
			<li v-if="!filteredOptions.length" style="text-align: center;" class="text-center">
				<slot name="no-options">Sorry, no matching options.</slot>
			</li>
			<li v-if="onGetMoreOptions && filteredOptions.length" style="text-align: center;" class="text-center">
